...
export const AddTeamMembers = ({
  ...
  setAddedMembers: React.Dispatch<React.SetStateAction<IUserType[]>>;
  ...
  const { data, status, error } = useSearchMemberQuery({
    competitionId: "123",
  ...
  useEffect(() => {
    if (data?.result.exist) {
      ...
      setIsShowUser(true);
      setUserInfo({ name: data.result.memberName, email: data.result.email });
    } else {
      setIsShowUser(false);
      setUserInfo({
        name: "",
